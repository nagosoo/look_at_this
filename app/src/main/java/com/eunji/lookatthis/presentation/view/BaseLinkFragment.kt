...
abstract class BaseLinkViewModel : ViewModel() {
    abstract val links: StateFlow<PagingData<LinkModel>>
    abstract val readUiState: StateFlow<UiState<LinkModel?>>
    abstract val bookmarkUiState: StateFlow<UiState<LinkModel?>>
...
abstract class BaseLinkFragment : Fragment() {
    ...
    lateinit var toggleBookmarkView: (LinkModel, Int) -> Unit
    val adapter by lazy { LinkAdapter(::readCallback, ::bookmarkCallback) }
    private var bookmarkItem: LinkModel? = null
    ...
    private fun subscribeReadState(
        readState: Flow<UiState<LinkModel?>>
    ) {
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
    ...
    private fun subscribeBookmarkState(
        bookmarkState: Flow<UiState<LinkModel?>>,
    ) {
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
    ...
    private fun renderRead(readState: UiState<LinkModel?>) {
        when (readState) {
            is UiState.None -> {}
            is UiState.Loading -> {}
    ...
    private fun renderBookmark(
        bookmarkState: UiState<LinkModel?>,
    ) {
        when (bookmarkState) {
            is UiState.None -> {}
            is UiState.Loading -> {}
